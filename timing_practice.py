...
def draw_text(
    ...
    text: str,
    pos: tuple[int, int],
    color: pygame.Color | tuple[int, int, int] = (232, 236, 241),
) -> pygame.Rect:
...
def draw_panel(
    ...
    border: tuple[int, int, int] = (65, 72, 84),
) -> None:
    pygame.draw.rect(surface, fill, rect, border_radius=8)
    pygame.draw.rect(surface, border, rect, width=1, border_radius=8)


def summarize(history: deque[Attempt]) -> tuple[int, int, int]:
...
def main() -> None:
    ...
    history: deque[Attempt] = deque(maxlen=MAX_HISTORY)
    ...
    while running:
        ...
        if now < next_render_time:
            ...
            continue
        next_render_time = now + FRAME_SECONDS
        ...
        draw_text(
            screen,
            small_font,
            "Input poll 1000 Hz   G edit goal   R reset   Esc quit",
            (34, 96),
            (166, 176, 190),
        )
        ...
        for index, item in enumerate(reversed(history), start=1):
            ...
            line = (
                f"{index:2}. {item.start_button:>5} -> {item.end_button:<5} "
